...
이 프로젝트를 통해 다음 질문에 코드 근거를 들어 답할 수 있습니다.

1. JWT를 발급하는 것과 Spring Security 인증을 완성하는 것은 어떻게 다른가?
2. 인증 실패(401)와 인가 실패(403)를 어디에서 처리해야 하는가?
3. Refresh Token 회전은 무엇을 막고, 현재 구현에는 어떤 한계가 있는가?
4. OAuth 제공 정보가 부족할 때 신규 가입 상태를 어떻게 안전하게 이어갈 것인가?
5. 선택적인 검색 조건을 QueryDSL로 어떻게 조합하고 페이징 count를 맞출 것인가?
6. 관리자 대시보드 집계를 애플리케이션이 아닌 DB에서 수행한 이유는 무엇인가?
7. 팀 프로젝트의 전체 성과와 개인 구현을 어떤 근거로 구분할 것인가?

## 10. 문서 범위

이 README의 기능 설명은 현재 저장소 코드와 Git 이력으로 확인되는 범위를 기준으로 작성했습니다. 발표 당시 운영 인프라는 별도 저장소를 포함했으며, 이 저장소만으로 VPC, NGINX, Elasticsearch, Kibana 구성을 재현할 수는 없습니다. 발표자료에 포함됐던 PostGIS와 pgvector 기반 기능도 현재 백엔드 코드에 구현되어 있지 않습니다.
